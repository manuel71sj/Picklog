...
import { evaluateAlphaDataset, type AlphaDatasetRow } from "../packages/shared/src/index.ts";

test("30-row Alpha dataset meets safety, must-hit, review-label, and search thresholds", () => {
  const rows = JSON.parse(readFileSync(new URL("./fixtures/alpha-dataset.json", import.meta.url), "utf8")) as AlphaDatasetRow[];
